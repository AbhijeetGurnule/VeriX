...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        logger.debug("Authorization header value: {}", header);   // change from logger.info

        if (header != null && header.startsWith("Bearer ")) {
            // token  extract -> validate -> authentication create -> security context ke andar ser karunga.
            ...
            try {
                ...
                userRepository.findById(userUuid)
                        .ifPresent(user -> {
                            ...
                            // check for user is enable or not
                            if (user.isEnabled()) {
                                // We got user from database here
